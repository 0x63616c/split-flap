...
- `just cad` = help. All cad cmds under it: `just cad dev [model]` = dev loop (viewers :3939 assembly / :3940 focus, cmux panes, save-watcher; model pins focus), `just cad list|down` = menu | stop, `just cad export [part]` = STL(s) to cad/export/ (no part = all), `just cad test|install`.
