...
- **`../../firmware/micropython-spike/bench_board.py`** — throwaway firmware
  command loop so the board is driveable. `just up` copies it onto the board
...
```
python tools/bench/bench_ui.py    # or: just bench
```

Real board at the bench (pulls pyserial/mpremote via `uv run --with`):

```
just up      # free port -> flash the command loop -> reset -> drive it
```

Port auto-picks the first `cu.usbmodem*`; override with `just up port=/dev/cu.usbmodemXXXX`.

Then type: a glyph (`A`–`Z`, `0`–`9`, empty = blank) to drive there, or a
command — `/help` lists them all (`/home`, `/nudge`, `/sethome`, `/quit`, …).
