...
What's fixed and what's ours: the magnet axes (all six at fin_hole_x)
...
Every tab is at least fin_flat_t thick, because every tab holds a
magnet. test_fins.py enforces that against magnet_locs() — golden tests
freeze whatever shape they are handed, so they will happily preserve a
pocket cut into thin air.

Bare geometry only — no pockets. shell.with_fins() unions these onto a
body and cuts the magnet pockets at magnet_locs().
"""
...
def _corner_tab_bottom():
    ...
    fin_flat_t thick throughout: a magnet needs pocket + floor behind it,
...
def _corner_tab_top():
    ...
    vendor used, because the ramp thins toward that edge and the pocket
    has width: measured at the axis this tab looks deep enough, but the
    outer lip of the bore ends up with only 4.03 behind it. The vendor
    could taper to 3 there — those tabs took screws, which don't care
    what's behind them.
    """
...
def magnet_locs():
    """Every magnet axis, posed on its mating face with +Z pointing into
    the module. THE latch interface: shell.py cuts each pocket down this
    axis, and every tab is built thick enough to take one, so a pocket
    can't be cut where there's nothing behind it.
    """
    out = []
    for y_s in (+1, -1):
        y = y_s * P.fin_hole_y
        out.append(Pos(P.fin_hole_x, y, 0))  # bottom corner, +Z inward
        out.append(
            Pos(P.fin_hole_x, y, P.unit_back_height) * Rot(180, 0, 0)
        )  # top corner, -Z inward
        out.append(
            Pos(P.fin_hole_x, y_s * P.unit_plate_h / 2, P.fin_stack_hole_z)
            * Rot(y_s * 90, 0, 0)
        )  # stack, inward off the y=+-59 face
    return out
